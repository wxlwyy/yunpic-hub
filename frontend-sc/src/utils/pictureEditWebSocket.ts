export default class PictureEditWebSocket {
  ...
  connect() {
    ...
    // 监听后端的消息事件（浏览器原生 WebSocket 收到消息）
    this.socket.onmessage = (event) => {
      const message = JSON.parse(event.data) // 解析 JSON
      console.log('收到消息:', message) // 拿到消息类型，比如 "EDIT_ACTION"

      // 根据消息类型触发对应事件
      const type = message.type
      this.triggerEvent(type, message)  // F. 触发分发器：谁订阅了 "EDIT_ACTION"，就通知谁
    }
  ...
  sendMessage(message: object) {
    if (this.socket && this.socket.readyState === WebSocket.OPEN) {
      this.socket.send(JSON.stringify(message))  // 把对象转成 JSON 字符串，通过网线发给 Java 后端
