    ...
    <div class="cropper-container">
      <div v-if="isTeamSpace" class="collab-status-bar">
        ...
        <div v-else class="idle-badge">
          <CheckCircleOutlined /> 暂无人编辑，点击“进入编辑”获取权限
        </div>
      </div>
      ...
      <div class="cropper-footer">
        <div class="edit-control-row" v-if="isTeamSpace">
          <a-space>
            <a-button v-if="canEnterEdit" type="primary" shape="round" @click="enterEdit">
              <template #icon><EditOutlined /></template> 进入编辑
            </a-button>
            <a-button v-if="canExitEdit" danger ghost shape="round" @click="exitEdit">
              <template #icon><LogoutOutlined /></template> 退出编辑
            </a-button>
          </a-space>
        </div>
        ...
        <div class="tool-bar">
          <a-space size="middle">
            <a-button-group>
              <a-button :disabled="!canEdit" @click="rotateLeft" title="向左旋转"><RotateLeftOutlined /></a-button>
              <a-button :disabled="!canEdit" @click="rotateRight" title="向右旋转"><RotateRightOutlined /></a-button>
            </a-button-group>
            <a-button-group>
              <a-button :disabled="!canEdit" @click="changeScale(1)" title="放大"><ZoomInOutlined /></a-button>
              <a-button :disabled="!canEdit" @click="changeScale(-1)" title="缩小"><ZoomOutOutlined /></a-button>
            </a-button-group>
            <a-button type="primary" :loading="loading" :disabled="!canEdit" @click="handleConfirm" class="confirm-btn">
              保存修改
            </a-button>
          </a-space>
        </div>
...
import {
  ...
  ZoomOutOutlined, EditOutlined, LogoutOutlined, CheckCircleOutlined
...
const initWebsocket = () => {
  ...
  websocket.on(PICTURE_EDIT_MESSAGE_TYPE_ENUM.EDIT_ACTION, (msg) => {
    ...
    switch (msg.editAction) {
      case PICTURE_EDIT_ACTION_ENUM.ROTATE_LEFT: cropperRef.value.rotateLeft(); break
      case PICTURE_EDIT_ACTION_ENUM.ROTATE_RIGHT: cropperRef.value.rotateRight(); break
      case PICTURE_EDIT_ACTION_ENUM.ZOOM_IN: cropperRef.value.changeScale(1); break
      case PICTURE_EDIT_ACTION_ENUM.ZOOM_OUT: cropperRef.value.changeScale(-1); break
    }
  })
...
onUnmounted(() => { if (websocket) websocket.disconnect() })
...
const rotateLeft = () => { cropperRef.value.rotateLeft(); editAction(PICTURE_EDIT_ACTION_ENUM.ROTATE_LEFT) }
const rotateRight = () => { cropperRef.value.rotateRight(); editAction(PICTURE_EDIT_ACTION_ENUM.ROTATE_RIGHT) }
const changeScale = (num: number) => { cropperRef.value.changeScale(num); editAction(num > 0 ? PICTURE_EDIT_ACTION_ENUM.ZOOM_IN : PICTURE_EDIT_ACTION_ENUM.ZOOM_OUT) }

const handleConfirm = () => {
  cropperRef.value.getCropBlob((blob: Blob) => {
    handleUpload({ file: new File([blob], (props.picture?.name || 'img') + '.png', { type: blob.type }) })
  })
}
...
<style scoped>
.cropper-container { display: flex; flex-direction: column; gap: 16px; }
.collab-status-bar { height: 32px; display: flex; justify-content: center; align-items: center; }
.editing-badge { display: flex; align-items: center; gap: 8px; background: #fff1f0; border: 1px solid #ffa39e; padding: 4px 12px; border-radius: 20px; color: #cf1322; font-size: 13px; font-weight: 600; }
...
.cropper-workspace { background: #1e293b; padding: 20px; border-radius: 12px; }
