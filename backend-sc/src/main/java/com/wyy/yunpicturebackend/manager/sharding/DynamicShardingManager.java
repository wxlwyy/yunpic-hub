...
//@Component
@Slf4j
public class DynamicShardingManager {  // 在项目启动时（或动态触发时），强行修改分库分表的内部配置，将固定的picture逻辑表改为动态的所有列表
    ...
    private Set<String> fetchAllPictureTableNames() {
        ...
        return tableNames;  // 比如拼装表名列表：["picture", "picture_100", "picture_200"]
    }
    ...
    private void updateShardingTableNodes() {
        ...
        // newActualDataNodes类似于yun_picture.picture_100,yun_picture.picture_200
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "yun_picture." + tableName) // 确保前缀合法
                .collect(Collectors.joining(","));
        log.info("动态分表 actual-data-nodes 配置: {}", newActualDataNodes);

        ContextManager contextManager = getContextManager();  // ContextManager。这是 SS（分库分表） 的大脑，管着所有配置。
