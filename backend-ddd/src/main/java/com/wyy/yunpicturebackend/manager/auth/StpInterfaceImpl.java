...
import static com.wyy.yunpicture.domain.user.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        // 判断 loginType，仅对类型为 "space" 进行权限校验，这里将三种空间视为空间体系
        if (StpKit.SPACE_TYPE.equals(loginType)) {
            // 管理员权限列表，表示权限校验通过
            List<String> ADMIN_PERMISSIONS = spaceUserAuthManager.getPermissionKeyListByRoleKey(SpaceRoleEnum.ADMIN.getValue());
            // 获取上下文对象（图片，空间用户关联，空间三种id）
            ...
            /*if (spaceUser != null) {
                return spaceUserAuthManager.getPermissionKeyListByRoleKey(spaceUser.getSpaceRole());
            } */
            ...
            if (spaceUserId != null) {
                ...
                SpaceUser loginSpaceUser = spaceUserService.lambdaQuery()
                        ...
                        .one();
                if (loginSpaceUser == null) {  // 没有当前用户的空间用户关联信息，当前用户就没权限
                    return new ArrayList<>();
                }
                // 这里会导致管理员在私有空间没有权限，可以再查一次库处理
                return spaceUserAuthManager.getPermissionKeyListByRoleKey(loginSpaceUser.getSpaceRole());
            }
            ...
            if (spaceId == null) {
                ...
                // 图片 id 也没有，则默认通过权限校验
                if (pictureId == null) {  // 查看公共图库
                    return ADMIN_PERMISSIONS;
                }
                Picture picture = pictureService.lambdaQuery()
                        .eq(Picture::getId, pictureId)
                        .select(Picture::getId, Picture::getSpaceId, Picture::getUserId)   // 没spaceUserId有pictureId查空间id和用户id图片id
                        .one();
                if (picture == null) {
                ...
                if (spaceId == null) {
                    if (picture.getUserId().equals(userId) || userApplicationService.isAdmin(loginUser)) {
                        ...
                        // 不是自己的图片，仅可查看
                        return Collections.singletonList(SpaceUserPermissionConstant.PICTURE_VIEW);  // 查看公共图库
                    }
                }
            }
            // 获取 Space 对象
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        String contentType = request.getHeader(Header.CONTENT_TYPE.getValue());//Header.CONTENT_TYPE.getValue() 返回的就是字符串 "Content-Type"
        ...
        if (ObjUtil.isNotNull(id)) {
            String requestUri = request.getRequestURI();  // 例如: "/api/picture/delete"
            String partUri = requestUri.replace(contextPath + "/", ""); //去掉 "/api/"，得到: "picture/delete"
            String moduleName = StrUtil.subBefore(partUri, "/", false); // 取第一段，得到: "picture"
            switch (moduleName) {
                case "picture":
